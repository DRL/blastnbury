...
my ($query_file) = ("");
my ( $blast_switch, $hit_threshold, $display_threshold, $report_threshold,
    $help, $man )
    = ( 1, 100, 15, 5, "", "" );
...
# ----------------------------------------------------------------------------
my @parameters
    = ( "-q", "-blast", "-noblast", "-hit", "-hits", "-display", "-save" )
    ;    #&VALIDATE_ARGS uses this for validation of parameters
...
unless ( $hit_threshold > $display_threshold
    && $display_threshold > $report_threshold )
{
    die
...
while ( $query_object = $query_file_object->next_seq ) {
    if ( defined $query_object ) {
        ...
        if ( $seq_id =~ /^[A-Za-z0-9]{3}[_]{1}[A-Za-z0-9]{4}[_]{1}/ ) {
            ...
            $data{$seq_id}{'query'}{'name'}
                = substr( $seq_id, 0, 8 );    # XXX_0000_
            $data{$seq_id}{'query'}{'region'}
                = substr( $seq_id, 9, 3 );    # XXX_0000_
            $data{$seq_id}{'query'}{'seq'}    = $query_object->seq;
...
if ( scalar @illegal_sequence_identifiers >= 1 ) {
    foreach (@illegal_sequence_identifiers) {
        print strftime( "[%H:%M:%S]", localtime )
            . " - Illegal sequence name in "
            . $_ . "\n";
    }
    die strftime( "[%H:%M:%S]", localtime )
        . " - Please correct the sequence names in "
        . $query_file . "\n";
}
...
#
    foreach my $query_id ( sort keys %data ) {
        ...
        print strftime( "[%H:%M:%S]", localtime ) . " = "
            ...
            . $i . " of "
            . $number_of_sequences . ") ["
            . &TIME_DIFFERENCE($start_of_blast)
            . "sec] \n";
        $start_of_blast = time();
        $i++;
    }
    ...
    print strftime( "[%H:%M:%S]", localtime )
        . " - Skipping BLAST search. Trying to find '*_BLAST.txt' file(s)...\n";
}
...
foreach my $id ( sort keys %data ) {
    ...
    my @fields            = (
        ...
        "send",     "eval",     "alen", "nident",
        "pident",   "mismatch", "gaps", "frames"
    );    # fields of the CSV file (determined by &BLAST)
    my $blast_file = $id . "_BLAST.txt";

    open( BLAST_HITS, "<" . $blast_file )
        or die "   [ERROR] - " . $blast_file . " could not be opened\n";
    while (<BLAST_HITS>) {
        chomp;
        if (m/^#/) {
            $datestamp = $_;
...
# Checks whether an ACC has already been read before from the files (i.e. multiple HSPs per ACC)
                if ( defined $data{$id}{'hits'}{$hit_acc} ) {
                    $num_of_hsps++;
                    $data{$id}{'hits'}{$hit_acc}{'main'}{'hsps'}
                        = $num_of_hsps + 1;
                    ...
                    for ( my $k = 0; $k < scalar(@fields); $k++ ) {
                        ...
                        if (   $fields[$k] eq "bitscore"
                            || $fields[$k] eq "alen"
                            || $fields[$k] eq "nident"
                            || $fields[$k] eq "mismatch"
                            || $fields[$k] eq "gaps" )
                        { # values in these fields can be added when multiple HSPs
                            $data{$id}{'hits'}{$hit_acc}{'main'}
                                { $fields[$k] }
                                = $data{$id}{'hits'}{$hit_acc}{'main'}
                                { $fields[$k] } + $hit_data[$k];
                            $data{$id}{'hits'}{$hit_acc}{'hsps'}
                                {$num_of_hsps}{ $fields[$k] } = $hit_data[$k];
                        }
                        if (   $fields[$k] eq "sstart"
                            || $fields[$k] eq "send"
                            || $fields[$k] eq "qstart"
                            || $fields[$k] eq "qend"
                            || $fields[$k] eq "eval"
                            || $fields[$k] eq "pident"
                            || $fields[$k] eq "frames" )
                        {    # values in these fields unique for each HSPs
                            $data{$id}{'hits'}{$hit_acc}{'hsps'}
                                {$num_of_hsps}{ $fields[$k] } = $hit_data[$k];
                        }
        ...
        # For first hit for an ACC (first hit of multiple HSPs, or single HSP)
                    ...
                    $data{$id}{'hits'}{$hit_acc}{'main'}{'hsps'}
                        = $num_of_hsps + 1;
                    $data{$id}{'hits'}{$hit_acc}{'main'}{'qlen'}
                        = $data{$id}{'query'}{'length'};
                    $data{$id}{'hits'}{$hit_acc}{'link'}
                        = "http://www.ncbi.nlm.nih.gov/nuccore/" . $hit_acc;
                    $data{$id}{'hits'}{$hit_acc}{'length'} = $hit_length;
                    $data{$id}{'hits'}{$hit_acc}{'desc'}
                        = &TRUNCATE_DESC( $hit_desc,
                        $truncate_desc_to_length );
                ...
                $data{$id}{'hits'}{$hit_acc}{'main'}{'differences'}
                    = $data{$id}{'hits'}{$hit_acc}{'main'}{'mismatch'}
                    + $data{$id}{'hits'}{$hit_acc}{'main'}{'gaps'};
                $data{$id}{'hits'}{$hit_acc}{'main'}{'cov'}
                    = 100 * ( $data{$id}{'hits'}{$hit_acc}{'main'}{'alen'}
                        - $data{$id}{'hits'}{$hit_acc}{'main'}{'gaps'} )
                    / $data{$id}{'hits'}{$hit_acc}{'main'}{'qlen'};
            }
        }
    ...
    print strftime( "[%H:%M:%S]", localtime ) . " = "
        ...
        . $sequence_counter . " of "
        . $number_of_sequences . ") ["
        . &TIME_DIFFERENCE($start_of_get_desc)
        . "sec] \n";
}
...
# Header for CSV file
open( CSV_FILE, ">" . $query_file . "_results.csv" )
    || die "Cannot write to " . $query_file . "_results.csv\n";
print CSV_FILE "#BEA_CODE,region,length,seq,date";
for ( my $j = 1; $j < $report_threshold + 1; $j++ ) {
    print CSV_FILE ",HIT_ACC_"
        . $j
        . ",HIT_DESC_"
        . $j
        . ",NUM_OF_HSPS_"
...
foreach my $id ( sort keys %hash_for_sorting ) {
    ...
    foreach
        my $hit_number ( sort { $a <=> $b } keys %{ $hash_for_sorting{$id} } )
    {
    ...
    my @user_input
        = &ASK_FOR_USER_INPUT( $display_threshold, $report_threshold );
    my $sort_order;
    while ( scalar(@user_input) == 1 ) {
        ...
        if ( $user_input[0] eq 'b' ) {
            foreach my $position ( keys %{ $hash_for_sorting{$id} } ) {
                ...
                $hash_of_fields{$current_acc}
                    = $data{$id}{'hits'}{$current_acc}{'main'}{'bitscore'};
            }
            ...
            foreach my $position ( keys %{ $hash_for_sorting{$id} } ) {
                my $current_acc = $hash_for_sorting{$id}{$position};
                $hash_of_fields{$current_acc}
                    = $data{$id}{'hits'}{$current_acc}{'main'}{'pident'};
            }
            $sort_order = "descending";
        }
            ...
            foreach my $position ( keys %{ $hash_for_sorting{$id} } ) {
                my $current_acc = $hash_for_sorting{$id}{$position};
                $hash_of_fields{$current_acc}
                    = $data{$id}{'hits'}{$current_acc}{'main'}{'cov'};
            }
            $sort_order = "descending";
        }
            ...
            foreach my $position ( keys %{ $hash_for_sorting{$id} } ) {
                my $current_acc = $hash_for_sorting{$id}{$position};
                $hash_of_fields{$current_acc}
                    = $data{$id}{'hits'}{$current_acc}{'main'}{'differences'};
            }
            ...
            foreach my $position ( keys %{ $hash_for_sorting{$id} } ) {
                my $current_acc = $hash_for_sorting{$id}{$position};
                $hash_of_fields{$current_acc}
                    = $data{$id}{'hits'}{$current_acc}{'main'}{'alen'};
            }
            $sort_order = "descending";
        }
            ...
            foreach my $position ( keys %{ $hash_for_sorting{$id} } ) {
                my $current_acc = $hash_for_sorting{$id}{$position};
                $hash_of_fields{$current_acc}
                    = $data{$id}{'hits'}{$current_acc}{'main'}{'eval'};
                $sort_order = "ascending";
            }
        }
        my @array_of_values;
        ...
        my @sorted_array_of_values
            = &SORT_BY( \$sort_order, \@array_of_values );
        my $rank = 1;
        foreach my $value_of_sorted_array_of_values (@sorted_array_of_values)
        {
            foreach my $acc_of_hash_of_fields ( keys %hash_of_fields ) {
                if ( $value_of_sorted_array_of_values ne "N/A" ) {
                    if ( $value_of_sorted_array_of_values
                        == $hash_of_fields{$acc_of_hash_of_fields} )
                    {
                        $hash_for_sorting{$id}{$rank}
                            = $acc_of_hash_of_fields;
    ...
    if ( scalar(@user_input) == $report_threshold ) {
        open( CSV_FILE, ">>" . $query_file . "_results.csv" )
            || die "Cannot write to " . $query_file . "_results.csv\n";
        print CSV_FILE &PRINT_SEQ_DATA_TO_FILE( \$datestamp,
            \%{ $data{$id} } );
        close(CSV_FILE);
        my %hash_of_selected_hits;
        for ( my $m = 0; $m < $report_threshold; $m++ ) {
            my $number_of_selected_hit = $user_input[$m];
            my $accession_of_selected_hit
                = $hash_for_sorting{$id}{$number_of_selected_hit};
            open( CSV_FILE, ">>" . $query_file . "_results.csv" )
                || die "Cannot write to " . $query_file . "_results.csv\n";
            print CSV_FILE &PRINT_HIT_TO_FILE(
                \%{ $data{$id}{'hits'}{$accession_of_selected_hit} } );
            close(CSV_FILE);
        }
        open( CSV_FILE, ">>" . $query_file . "_results.csv" )
            || die "Cannot write to " . $query_file . "_results.csv\n";
...
sub PRINT_LOGO {
    ...
    if ( $_[ 0 eq 'help' ] ) {
        $additional_string
            = "\t\t###      (more under blastnbury.pl -help -man)   ###\n";
    }
    print("\n\t\t####################################################\n");
    print("\t\t###                                              ###\n");
...
sub VALIDATE_ARGS {
    ...
    foreach my $argument (@arguments) {
        ...
        unless ( ( $argument =~ m/^-/ && defined $parameters{$argument} )
            || ( $argument !~ m/^-/ && &CHECK_FILE($argument) )
            || ( $argument !~ m/^-/ && $argument =~ /^[+-]?\d+$/ ) )
        {
            return 0;
        }
    }
...
sub BLAST {
    ...
    my $blast_call
        = "blastn -query "
        . $temp_file
        . " -outfmt '6 stitle sacc slen bitscore qstart qend sstart send evalue length nident pident mismatch gaps frames' -remote -db nt -max_target_seqs "
        . $hit_threshold
        . " -out "
        . $outfile;
    system($blast_call);
...
sub PRINT_RESULT_HEADER {
    ...
    print
        "----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n";
    print
        "[ #] Description \t\t\t\t\tLength\tHSPs\tID\tCov\tEval\tqstart\tqend\tframe\t∆nt\tgaps\tURL/Comments\n";
    print
        "----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n";
}
...
sub PRINT_HIT {
    ...
    my $total_cov   = sprintf( "%.2f%%",
        100 * ( ( $hit_alen - $gaps ) / $query_length ) );
    ...
    foreach my $hsp ( sort keys %{ $hit_hash{'hsps'} } ) {
        ...
        my $hsp_pident = sprintf( "%2s",
            sprintf( "%.1f%%", $hit_hash{'hsps'}{$hsp}{'pident'} ) );
        my $hsp_gaps = $hit_hash{'hsps'}{$hsp}{'gaps'};
        my $hsp_cov  = sprintf(
            "%.1f%%",
            100 * (
                ( $hit_hash{'hsps'}{$hsp}{'alen'} - $hsp_gaps )
                / $query_length
            )
        );
        ...
        $hsp_diff = $hit_hash{'hsps'}{$hsp}{'alen'}
            - $hit_hash{'hsps'}{$hsp}{'nident'};
        $hsp_frames =~ s/1/+/g;
        $hsp_frames =~ s/0/-/g;
        ###
        printf(
            "%4s\t%6s\t%6s\t%4s\t%4s\t%4s\t%3s\t%4s\t%4s\t",
            $num_of_hsps, $hsp_pident, $hsp_cov,
            $hsp_eval,    $hsp_qstart, $hsp_qend,
            $hsp_frames,  $hsp_diff,   $hsp_gaps
        );
...
sub ASK_FOR_USER_INPUT {
    ...
    my $message_two
        = "or select " . $report_threshold . " hits for the report : ";
    ...
    while ($red_flag) {
        ...
        elsif ( scalar(@input) == $report_threshold )
        {    #array in length of report_threshold
...
sub PRINT_SEQ_DATA_TO_FILE {
    ...
    return
          $seq_data{'query'}{'name'} . ","
        . $seq_data{'query'}{'region'} . ","
        . $seq_data{'query'}{'length'}
        . ",", $seq_data{'query'}{'seq'} . "," . $date;
}
...
sub PRINT_HIT_TO_FILE {
    ...
    my $temp_hit_cov  = sprintf( "%.1f%%",
        100 * ( $temp_hit_alen - $temp_hit_gaps ) / $temp_hit_qlen );
